...
\fixme{Describe chapter \ref{chapter:discussion}.}
...
The target tempo, $v_t$, is taken as an exponential moving average
...
To make the tempo change $\Delta v$,
during the time $t_c$, we define
\begin{equation}
v_t(t) = \frac{\Delta v t}{t_c}.
\end{equation}
Thus, the difference to our target tempo
\begin{equation}
v'_t(t) = \frac{\Delta v t}{t_c} - \Delta v,
\end{equation}
and the difference in position due to this
tempo difference can be solved from the integral
\begin{equation}
\int v'_t \, \mathrm{d}t =
\Delta v \left( \frac{t^2}{2 \, t_c} - t \right) + C.
\end{equation}
The cumulative difference in position due to $v_t(t)$
over $t_c$
\begin{equation}
\Delta b_{v_t} =
\int_t^{t + t_c} v'_t \, \mathrm{d}t =
-\frac{\Delta v \, t_c }{2}.
\end{equation}


To have zero effect on the final tempo,
we define
\begin{equation}
v_b(t) = a \sin \left( \frac{\pi t}{t_c} \right),
\end{equation}
where $a$ is the correction coefficient.
The cumulative position difference can be solved from
the integral
\begin{equation}
\int v_b \, \mathrm{d}t =
\frac{a t}{\pi} \left( 1 - \cos \left( \frac{\pi t}{t_c} \right) \right),
\end{equation}
giving the total difference in position
\begin{equation}
\Delta b_{v_b} = \int_t^{t + t_c} v_b \, \mathrm{d}t =
\frac{2 \, a \, t_c}{\pi}
\end{equation}
To correct the position after $t_c$,
$v_b(t)$ will need to compensate for both
$\Delta b$ and counter $\Delta b_{v_t}$.
Thus we can solve $a$ from the equation
\begin{equation}
\Delta b_{v_b} = \Delta b - \Delta b_{v_t},
\end{equation}
giving the solution
\begin{equation}
