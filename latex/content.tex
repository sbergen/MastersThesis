...
To make the gross tempo change
over $t_c$ equal $\Delta v$,
we define
\begin{equation}
v_t(t) =
\begin{cases}
...
\end{equation}
Similarly to $b_{v_t}$,
the gross difference in position that cumulates over $t_c$
\begin{equation}
...
$v_b(t)$ will need to both compensate for
$\Delta b$ and counter $\Delta b_{v_t}$,
giving the equation
\begin{equation}
\Delta b_{v_b} = \Delta b - \Delta b_{v_t}.
\end{equation}
Using equation \ref{eq:non-linear_tempo_func_total},
we can solve $a$ as
\begin{equation}
