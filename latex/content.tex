...
Both studies used Savitzky-Golay filtering (polynomial fitting) \cite{savitzky1964}
...
Because of the cyclic nature of conducting,
some methods for averaging over time were required.
When selecting parameters to the averaging methods,
...
is to pre-calculate $\mathbf{X}$,
with $x_{\lfloor \frac{n}{2} \rfloor} = 0$.
The value and derivatives are calculated from the polynomial
\begin{equation}
\widehat{y}(x) =
