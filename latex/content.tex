...
which defines the amount of change in position.
...
would cause a very non-musical result,
as the tempo change would be temporarily very large.
This is why $t_c$ is selected as the time between
...
The tempo adjustment method described in section
\ref{sec:tempo_adjustment}
is very strict,
...
as the offset is directly affected by
the tempo adjustment function.
This would cause feedback in the system,
with hard to predict results.
Instead, only part of the beat offset is compensated for.
This does apply to the offset caused by the tempo change
