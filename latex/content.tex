...
Modeling this complex conductor-musician interaction
...
were recognized by over 70\% of the subjects,
with the recognition rate having a strong correlation
with the experience level of the musicians.

While some conducting gestures can be easily analyzed,
...
differs depending on its position in the score.
...
An example setup of the final result is displayed
in figure \ref{fig:setup} for reference.
Previous conductor follower systems,
motion tracking methods,
score following methods,
and expressive synthesis methods
are studied and analyzed,
and a set of methods for an implementation
is devised based on the study.
These methods are used to implement the system,
and further details emerging from the implementation
...
including a set of speakers, a Microsoft Kinect,
and a PC running Conductor Follower along with
a synthesis environment, Vienna Ensemble \cite{vsl}.}
\label{fig:setup}
\end{center}
\end{figure}
...
the methods used for analyzing the gesture data,
...
the overview here is kept brief.
...
the reader is asked to refer to \cite{Fabiani2012}.
...
The methods used for analyzing the raw data
...
the methods used for analysis are somewhat similar,
using methods such as simple heuristics, HMMs or ANNs.
...
Fabiani et al. \cite{Fabiani2012} define three levels of performance control:
...
the system is not very sensitive to the input,
...
were to be modeled as closely as possible,
...
\end{enumerate}
Out of these goals,
only the \spellRefOrdinal{item:usable_without_equipment} one
is easily evaluated,
...
averaging over time,
in order to extract features that apply
...
for analyzing the movement at time $t_i$,
...
\]
%$M \in \{ \mathrm{None}, \mathrm{Dip}, \mathrm{Peak} \}.$
A change in direction can be detected as a change of sign in the first derivative:
\begin{equation}
m_i =
\begin{cases}
\mathrm{None} & \text{if } \sgn{f'_{i-1}} = \sgn{f'_{i}} \\
\mathrm{Dip} & \text{if } \sgn{f'_{i-1}} < \sgn{f'_{i}} \\
\mathrm{Peak} & \text{if } \sgn{f'_{i-1}} > \sgn{f'_{i}} ,
\end{cases}
\end{equation}
where $f'_i$ is the first derivative of the
observed function at time $i$.
...
while still offering good averaging.
...
High values of $n$ provide a smoother output,
but do not react to sudden decreases in magnitude as fast.
...
The approach taken here is thus not
...
from the motion features of the conductor's hands.
...
the conducted experimentation would suggest that $v_y$ and $a_t$
...
using only the vertical position of the hand was finally chosen.
...
are concluded in figure \ref{fig:start_gesture}
and in the list below:
\begin{itemize}
\item A long enough immobile state ($\Delta t_i$ greater than threshold) has been detected not too long ago.
...
which often interprets changes in
the shape or angle of the hand
as forward or backward movements.
These "false" movements produce unexpected
results if included in the analysis.

\subsection{Windowed Trajectory Length}

The windowed trajectory length measures
...
This mapping is based on the conducted tempo of the piece,
...
\item Extra beats in "unexpected" positions are not acted upon. \label{follow_point:unexpected}
\item Sudden changes in tempo are not immediately followed.
\end{enumerate}
...
In this chapter,
we first formalize the different time bases used,
...
and is referred to as physical time,
...
\begin{enumerate}
\item Musical time has the base unit of \textit{beats}, but it also includes a time varying derived unit of \textit{bars}. Any position in the score can thus be expressed as either a beat offset, or a combination of bars and beats. Its dimension is denoted with $B$ and quantities with $b$.
\item Score time has the base unit of \textit{seconds}. This is an offset from the beginning of the score if the original tempo is followed. Its dimension is denoted with $\Phi$ and quantities with $\varphi$.
\end{enumerate}
A score position is thus defined as
$ \tupledef{P}{b, \varphi}, $
and variables are denoted with a $p$.

Mapping between physical time and score positions is denoted with
...
may produce results that differ
...
and a beat in a pattern be
\begin{equation}
q(i, j) = -\vert \mathbf{h}_i - \mathbf{p}_j \vert.
\end{equation}

\item Let $\alpha$ be such that $q(1, \alpha)$ is maximized,
...
\begin{equation}
\mathbf{h}'_i = \mathbf{h}_i + s(\mathbf{h}_i - \mathbf{h}_1),
\end{equation}
...
it scores better than its neighboring beats in the winning beat pattern
...
which attempt to correct the offsets
so that the playback matches the conducting
as well as possible.
The change in tempo is realized by adjusting
...
A visualization of the tempo function is presented
...
it does not immediately match the target tempo.
...
accumulates over the catchup time $t_c$,
...
-\frac{\Delta v \, t_c }{2}.
\end{equation}
This difference in position needs to be taken into account
when defining $v_b(t)$,
...
as defined in equation \eqref{eq:tempo_function_offset_function},
...
However, since $\Delta v$ is finite in most cases,
we can see that the coefficient $\frac{\Delta v}{t_c}$
in equation \eqref{eq:tempo_function_tempo_correction}
approaches infinity,
as $t$ approaches $t_c'$:
\begin{equation}
...
\, \Delta v \neq 0.
\end{equation}
Thus it is not feasible to use equation \eqref{eq:tempo_change_bad_tc},
...
section \ref{sec:tempo_function} is non-linear,
...
the timing of the detected beats often have some
...
causes a large temporary change in tempo.
...
(equation \eqref{eq:linear_tempo_change_pos_diff}),
...
\eqref{eq:non_linear_coef}.

The fraction discussed above,
together with the tempo filter cutoff time
presented in equation \eqref{eq:tempo_filter_weight_coef},
...
have values within \ccinterval{0, 1},
...
\ccinterval{v_{min}, v_{max}}, where $v_{min} > 0$,
in order to neither mute notes nor exceed the maximum velocity.

It is worth noting,
that the values for all the parameters
in the conductor context,
have already been scaled and clamped
to be in the range \ccinterval{0, 1}.
...
in equation \eqref{eq:veclocity_construction} instead.
...
while \textit{length} is emphasized.
As the length parameter describes the
longest possible note,
the distance function for the weight also has to be adjusted
...
to the sum in equation \eqref{eq:patch_distance},
...
All the features are presented in figure \ref{fig:visualization}.
...
\end{center}
\caption{
Visualization screenshot, showing
movement trace (red line),
spatial beat position (green dot),
current bar phase (turquoise segment),
beat offsets (red and greed segments),
and depth sensor data (grayscale).
}
...
as the bars progress to their ends.
...
Further details regarding the source code
availability and compatibility can be found
in appendix \ref{app:source_code_availability}.
...
It was used for its VST plugin wrapper,
MIDI file I/O,
...
Missing a deadline means that the missed block is not played back,
...
more to do with the algorithmic complexity
of the methods used,
...
makes the application a multirate, multithreaded system.
...
Handling time in a multirate, multithreaded system,
is not a trivial task.
The motion capture thread is in
a blocking state most of the time,
waking up at a 30 Hz frequency to process data
...
The plugin thread, however,
requires jitter correction to prevent timing problems.
...
\Delta \tau = \tau_e - \tau_b
\end{equation}
differs from the theoretical block length
...
\end{equation}
where $t_e$ is the event time.
Using equation \eqref{eq:theoretical_block_len},
...
These are mostly related to lock-free programming techniques.
...
a dynamically resizable container (such as std::vector) can be used,
while buffers used during playback mostly use a statically sized ringbuffer
to guarantee memory allocation free operation.
...
and checks for new tasks and executes them repeatedly,
until terminated.
The thread blocks as necessary to not exceed the
maximum execution interval.

\subsection{Lock-Free Ringbuffer}

The lock-free ringbuffer provided by
the Boost.Lockfree library \cite{BoostLockfree}
is used for most of the lock-free inter-thread communication.
...
It is an essential utility for lock-free multithreaded programming.

\subsection{Chen \& Burns Buffer}

While the lock-free ringbuffer is a good solution
for event-like data,
better mechanisms exist for sharing a single
...
The mechanism allows lock-free access for
...
polynomial regression filter is provided.
...
and evaluating the polynomial at the center value
...
used in equation \eqref{eq:sg_design_matrix}.
This change requires a correction to equation \eqref{eq:sg_coefs}:
...
The formats are described here briefly.
...
\item[Score events] A list of score events.
\end{description}
...
and for beat patterns section \ref{sec:beat_pattern_definition_format}.
While the format supports having
different types of score events,
only one type of score event was implemented.
...
\end{description}
The event causes the catchup time $t_c$,
...
A low sensitivity leads to a long $t_c$,
...
and a few informal testing sessions with 
...
The system is evaluated from
\spellRef{item:last_evaluation_perspective}
different perspectives:
\begin{enumerate}
\item The overall architecture,
\item the implemented feature set, and
...
The architecture was designed largely based on the
...
implementation modules is usually good,
...
Sousa \cite{sousa1988} lists eight different
conducting emblem categories:
...
the next section. % \ref{sec:feature_quality}.
...
to drift off badly,
and is a much more severe problem
...
as the system does not miss notes
...
as conductors often do not follow
...
one of the most likely parts of the system
...
has been used earlier \cite{Kirke2012}.
...
cause a bottleneck to the capabilities of the whole system.
...
they also require work in other parts of the system.
...
In its current form,
the use cases for the system are somewhat limited,
due to the purely academic nature of the implementation.
However, there are other potential use cases for the system,
which would require some modifications or extensions to the system.
Some of them are discussed in this section.
Others, such as use in conducting classes,
would require more research into what features would
actually be useful.

\paragraph*{Conduct your own recordings}
One use case for the system is being able to
record the result of your conducting,
for the purpose of listening to it later on.
Given a well made score and high quality synthesis environment,
the recordings can reach a high quality.
Listening to the resulting recordings could be beneficial
for conductors or composers evaluating different interpretations,
or for plain entertainment purposes.
Extending the idea to the internet,
these recordings could also be shared among friends,
or one could even hold virtual conducting competitions
where the recordings are shared and judged online.
Since currently the audio is output from a VST plugin,
recording it with a third party software is possible.
However, it would be possible to make this task
a lot easier by building the functionality into the system.
Especially if the recording is to be shared,
having one application for the whole task
would be a crucial improvement to the user experience.

\paragraph*{"Conductor Hero"}
Given the popularity of games like \textit{Guitar Hero},
where the player uses an instrument like controller
to simulate playing along with a band,
modifying the system to behave like a similar game
for conducting is a use case easily come up with.
However, the setting in such games is quite the opposite
of the current functionality of the conductor follower system --
where in the game, the user has to keep up with the system,
in the conductor follower, the system has to keep up with the user.
Nevertheless, there are several components in the system,
which could be reused in a game;
the required development effort would be very large though.
...
the end result met the main goals.
...
All in all,
the system is able to model
an essential subset of conductor-musician communication,
and produce a satisfying interactive experience
resembling conducting a real orchestra.
